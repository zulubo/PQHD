...
namespace PQHD {
	...
	public static class LayerMaskExtensions
	{
		...
		public static LayerMask GetPhysicsLayerMask(int layer)
		{
			...
			for (int i = 0; i < 32; i++)
			{
				// Check if the specified layer is set to collide with the current iteration layer in the physics settings
				if (!Physics.GetIgnoreLayerCollision(layer, i))
				{
					// If they collide, add the current iteration layer to the mask using bitwise OR
					mask |= (1 << i);
				}
			}
			return mask;
